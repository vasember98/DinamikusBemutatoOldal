// src/lib/dnd/actions/droppable.ts
import { drag, registerDroppable } from '../DragManager';
import type { DragState } from '../DragManager';

type Opts = {
  id: string;
  enabled?: boolean;                     // optional: turn droppable on/off
  label?: string;                        // optional: human-friendly name for announcements
  accepts?: (d: DragState) => boolean;   // optional: filter allowed drags
};
...
export function droppable(node: HTMLElement, opts: Opts) {
  ...
  return {
    update(next: Opts) {
      ...
      id = next.id ?? id;
      // preserve previous enabled when not provided
